...
fun main() {
    ...
    fun input(): INPUT10 {
        ...
        return File("2025/resources/input10.txt").readLines().map { line ->
    ...
    fun part1(input: INPUT10): String {
        ...
        val fewestButtonPresses = input.mapIndexed { idx, (startLeds, buttons, _) ->
            println("Processing ${idx+1}/${input.size}")
    ...
    fun part2(input: INPUT10): String {
        val fewestButtonPresses = input
            .map { (_, buttons, joltages) -> Pair(buttons, joltages.map { it.toDouble() })}
            .mapIndexed { idx, (buttons, joltages) ->
                println("Processing ${idx + 1}/${input.size}")


                val doubleBtns = buttons.map {
                    Array(joltages.size) {0.0}.also { arr ->
                ...
                val A = SimpleMatrix(joltages.size, buttons.size, false, *doubleBtns.flatten().toDoubleArray())

                val b = SimpleMatrix(joltages.size, 1, true, *joltages.toDoubleArray())

                try {
                    A.solve(b).elementSum()
                } catch (e: Exception) {
                    println("Failed to solve for ${idx+1}")
                    A.print()
                    b.print()
                    0.0
                }
